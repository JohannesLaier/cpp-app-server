...
std::string ErosResponseStaticFile::getMimeType(const std::string& fileName) {
	...
	ErosFile nFile(fileName);
	std::string mimeType = mimeTypes[nFile.extension()];
	if (mimeType.empty()) {
		mimeType = "text/html";
	}
	return mimeType;
...
std::string ErosResponseStaticFile::getFile(const std::string& fileName) {
	ErosFile nFile(fileName);

	ErosResponse response;
	response.setHeader("Content-Type", getMimeType(fileName));
	response.setContent(nFile.content());
